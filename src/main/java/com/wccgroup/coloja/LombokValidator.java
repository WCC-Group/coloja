...
public class LombokValidator
{
	...
	{
		return autoValidate(namespacePrefix, new ValidatorOptions());
	}
		...
		for (String className : new Reflections(namespacePrefix, new SubTypesScanner(false)).getAllTypes())
		{
			...
			if (classDefinition != null)
			{
				try
				{
					...
					validate(classDefinition, options);
	...
	{
		validate(clazz, new ValidatorOptions());
	}
		...
		throws InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException
	{
		if (Heuristics.isImmutable(clazz))
		{
			...
			ValueClassValidator.validate(clazz, options);
		}
		else
		{
